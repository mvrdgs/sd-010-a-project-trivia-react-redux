...
A página está hospedada no Git Pages, basta acessar o link: https://mvrdgs.github.io/trivia/

### Como jogar

* Após acessar a página, digite um e-mail em formato válido (exemplo@exemplo.com). O email não precisa existir e não será validado.
* Você deverá responder 5 perguntas que podem ser de múltipla escolha ou verdadeiro ou falso.
* Você terá 30 segundos para responder.
* Os segundos restantes são adicionados à pontuação.
* Se o tempo acabar, a resposta é considerada errada.
* Após responder as cinco perguntas o jogador será direcionado à página de feedbacks, onde poderá visualizar o ranking ou voltar ao início
* As informações relacionadas ao ranking ficarão salvos no Local Storage do seu navegador, portanto só serão visíveis para você.

OBS: A página de configurações ainda não está integrada ao projeto. É possível ver as opções, mas ela ainda não afeta o funcionamento do restante da aplicação.

### Proposta

 * Desenvolver um projeto utilizando React e Redux
 * Integrar a aplicação com a API que gera as perguntas
 * Desenvolver habilidades de trabalho em equipe
 * Manter o nível de padronização do código com auxílio do ESLint
